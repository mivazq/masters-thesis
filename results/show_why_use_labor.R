...
ggplot(variability, aes(x = value, fill = variable)) + 
    geom_density(data=variability[variable!="K_gr"], alpha = 0.5) + 
    geom_density(data=variability[variable=="K_gr"], aes(x = value, y = -(after_stat(density))), alpha = 0.5) +
    scale_fill_discrete(name = "Industry Group",  labels = c("Materials", "Labour", "Capital"), type = c("yellow","purple","black")) + 
