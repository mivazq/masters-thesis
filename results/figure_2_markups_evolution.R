#///////////////////////////////////////////////////////////////////////////////
# File name:		figure_2_markups_evolution.R
...
for (i_input in c("v", "m")) {
    ...
    ggsave(paste0(pathFig,sysdate,"_figure_2_aggregate_markup_",toupper(i_input),"_appendix.pdf"), width = 15, height = 10, device=cairo_pdf)
}

#  Plot only CD for input V (for main text)
ggplot(agg_mu[pf=="Translog Production" & input=="v" & variable %nin% c("sd")], aes(x = year, y = value, color = ind_group, linetype=variable)) +
    geom_line(linewidth=1.5) + geom_point(size=3) + geom_hline(yintercept=1) + 
    scale_linetype_manual(values= c(1,2), labels=c("Aggregate","Median")) +
    scale_color_discrete(labels = c(lgndlab1, lgndlab2, lgndlab3, lgndlab4), type = c("#f8766d","#00ba38","#619cff","#a3a500")) +
    scale_y_continuous(labels = function(x) fp(x, dig=1)) +
    xlab("Year") + ylab(expression(italic("\u03BC"["t"]))) +
    theme_bw() + guides(linetype="none") +
...
ggsave(paste0(pathFig,sysdate,"_figure_2_aggregate_markup_main.pdf"), width = 15, height = 10, device=cairo_pdf)
